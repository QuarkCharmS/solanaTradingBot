...
"""
async def swap(private_key, amount=None, input_token=SOLANA_MINT, output_token=SOLANA_MINT, slippageBps='50', priority_rate='0'):
    ...
    cmd = f"PRIVATE_KEY={private_key} INPUT_MINT={input_token} OUTPUT_MINT={output_token} AMOUNT={amount} SLIPPAGE_BPS={slippageBps} PRIORITY_RATE={priority_rate} node jupiter-swap-2.js"
    print(cmd)
...
"""
async def buy_with_sol(output_token, amount=20000000, private_key=PRIVATE_KEY, slippageBps='500', priority_rate='2000000'):
    print(amount)
    await swap(private_key, amount=amount, output_token=output_token, slippageBps=slippageBps, priority_rate=priority_rate)
...
"""
async def sell_to_sol(input_token, private_key=PRIVATE_KEY, amount=None, slippageBps='500', priority_rate='2000000'):
    await swap(private_key, amount=amount, input_token=input_token, slippageBps=slippageBps, priority_rate=priority_rate)




mint = input("token to buy/sell: ")

print(mint)


answer = input('sell? y/n -- answer: ')
if (answer == 'y'):

    asyncio.run(buy_with_sol(mint))

    input("Press enter to sell")

asyncio.run(sell_to_sol(mint))
